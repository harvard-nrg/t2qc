...
def do(args):
    ...
    # query T2w and corresponding vNav scan from XNAT
    ...
    for run,scansr in scans.items():
        if 'anat' in scansr:
            logger.info(f'getting anat run={run}, scan={scansr["anat"]}')
            t2qc.cli.get.get_anat(
                args,
                auth,
                run,
                scansr['anat'],
        ...
        if 'move' in scansr:
            logger.info(f'getting move run={run}, scan={scansr["move"]}')
            t2qc.cli.get.get_move(
                args,
                auth,
                run,
                scansr['move'],
        ...
        logger.debug(f'sub={args.sub}, ses={args.ses}')
        t2qc.cli.process.do(args)
